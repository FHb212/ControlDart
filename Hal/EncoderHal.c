...
extern I2C_HandleTypeDef hi2c2; // 使用的I2C句柄，根据实际情况修改
/// AS5600 functions
/* initialization */
...
HAL_StatusTypeDef AS5600_init(AS5600_TypeDef *handle)
{
    ...
    handle->i2c_handle = &hi2c2;
    if (!(handle->i2c_timeout))
    {
        handle->i2c_timeout = AS5600_I2C_TIMEOUT_DEFAULT;
    }
    if (!(handle->positive_rotation_direction))
    {
        handle->positive_rotation_direction = AS5600_DIR_CW;
    }
    if (!(handle->low_power_mode))
    {
        handle->low_power_mode = AS5600_POWER_MODE_DEFAULT;
    }
    if (!(handle->hysteresis))
    {
        handle->hysteresis = AS5600_HYSTERESIS_DEFAULT;
    }
    if (!(handle->output_mode))
    {
        handle->output_mode = AS5600_OUTPUT_STAGE_DEFAULT;
    }
    if (!(handle->PWM_frequency))
    {
        handle->PWM_frequency = AS5600_PWM_FREQUENCY_DEFAULT;
    }
    if (!(handle->slow_filter))
    {
        handle->slow_filter = AS5600_SLOW_FILTER_DEFAULT;
    }
    if (!(handle->fast_filter_threshold))
    {
        handle->fast_filter_threshold = AS5600_FAST_FILTER_DEFAULT;
    }
    if (!(handle->watchdog_timer))
    {
        handle->watchdog_timer = AS5600_WATCHDOG_DEFAULT;
    }

    if (AS5600_set_low_power_mode(handle, handle->low_power_mode) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (AS5600_set_hysteresis(handle, handle->hysteresis) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (AS5600_set_output_mode(handle, handle->output_mode, handle->PWM_frequency) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (AS5600_set_slow_filter(handle, handle->slow_filter) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (AS5600_set_fast_filter_threshold(handle, handle->fast_filter_threshold) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (AS5600_set_watchdog_timer(handle, handle->watchdog_timer) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (AS5600_write_config_register(handle) != HAL_OK)
    {
        return HAL_ERROR;
    }
    uint8_t magnet_status = 0;
    if (AS5600_get_magnet_status(handle, &magnet_status) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (!(magnet_status & AS5600_MAGNET_DETECTED) || (magnet_status & AS5600_AGC_MIN_GAIN_OVERFLOW) || (magnet_status & AS5600_AGC_MAX_GAIN_OVERFLOW))
    {
        return HAL_ERROR;
    }
    return HAL_OK;
...
HAL_StatusTypeDef AS5600_set_positive_rotation_direction(AS5600_TypeDef *const handle, const uint8_t direction)
{
    ...
    default:
        return HAL_ERROR; // invalid direction
    }
    return HAL_OK;
}
...
HAL_StatusTypeDef AS5600_set_hysteresis(AS5600_TypeDef *const handle, const uint8_t hysteresis)
{
    ...
    case AS5600_HYSTERESIS_3LSB:
        ...
        break;
    default:
        return HAL_ERROR; // invalid hysteresis mode
    }
    return HAL_OK;
}
HAL_StatusTypeDef AS5600_set_output_mode(AS5600_TypeDef *const handle, const uint8_t mode, uint8_t frequency)
{
    ...
    case AS5600_OUTPUT_STAGE_PWM:
        ...
        break;
    default:
        return HAL_ERROR; // invalid output mode
    }
    if (pwm)
    {
        switch (frequency)
        ...
        case AS5600_PWM_FREQUENCY_920HZ:
            ...
            break;
        default:
            return HAL_ERROR; // invalid PWM frequency
        }
    }
    return HAL_OK;
}
HAL_StatusTypeDef AS5600_set_slow_filter(AS5600_TypeDef *const handle, const uint8_t mode)
{
    ...
    case AS5600_SLOW_FILTER_2X:
        ...
        break;
    default:
        return HAL_ERROR; // invalid slow filter mode
    }
    return HAL_OK;
}
HAL_StatusTypeDef AS5600_set_fast_filter_threshold(AS5600_TypeDef *const handle, const uint8_t threshold)
{
    ...
    case AS5600_FAST_FILTER_10LSB:
        ...
        break;
    default:
        return HAL_ERROR; // invalid fast filter mode
    }
    return HAL_OK;
}
HAL_StatusTypeDef AS5600_set_watchdog_timer(AS5600_TypeDef *const handle, const uint8_t mode)
{
    ...
    case AS5600_WATCHDOG_ON:
        ...
        break;
    default:
        return HAL_ERROR; // invalid watchdog state
    }
    return HAL_OK;
}
